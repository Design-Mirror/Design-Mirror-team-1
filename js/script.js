...
function renderCart() {
  ...
  cartContent.innerHTML = `
    <div class="cart-item">
      ...
      <p class="cart-item-details">Fall Limited Edition Sneakers<br>$${price.toFixed(2)} × ${state.cartQuantity} <strong class="cart-item-total">$${total.toFixed(2)}</strong></p>
...
function renderCheckoutOrder(total = price * state.cartQuantity) {
  checkoutOrder.innerHTML = `<span>${state.cartQuantity} pair${state.cartQuantity === 1 ? "" : "s"} of sneakers</span><strong>$${total.toFixed(2)}</strong>`;
}
...
$("[data-checkout-done]").addEventListener("click", () => setCheckoutOpen(false));
